...
import { streamPRs, streamIssues } from '@/lib/api';
...
export default function Home() {
  ...
  const loadPRs = (owner: string, repo: string) => {
    abortControllerRef.current?.abort();
    setPRsLoading(true);
    setPRsError(null);
    setPRs([]);

    const controller = streamPRs(owner, repo, githubToken, {
      onMetadata: (totalCount) => {
        setPRsTotalCount(totalCount);
      },
      onSummary: (summary) => {
        setPRs((prev) => [...prev, summary].sort((a, b) => b.number - a.number));
      },
      onDone: () => {
        setPRsLoading(false);
      },
  ...
  const loadIssues = (owner: string, repo: string) => {
    ...
    const controller = streamIssues(owner, repo, githubToken, {
      onMetadata: (totalCount) => {
        setIssuesTotalCount(totalCount);
      },
      onSummary: (summary) => {
        setIssues((prev) => [...prev, summary].sort((a, b) => b.number - a.number));
      },
      onDone: () => {
  ...
  const handleTabChange = (tab: Tab) => {
    ...
    if (currentRepo) {
      if (tab === 'prs' && prs.length === 0 && !prsLoading) {
        loadPRs(currentRepo.owner, currentRepo.repo);
      } else if (tab === 'issues' && issues.length === 0 && !issuesLoading) {
        loadIssues(currentRepo.owner, currentRepo.repo);
      }
    }
  };
