...
export default function Home() {
  ...
  const [issuesTotalCount, setIssuesTotalCount] = useState<
    number | undefined
  >();
  ...
  const loadPRs = async (owner: string, repo: string) => {
    ...
    try {
      ...
      const errorMsg =
        err.response?.data?.detail || err.message || "Failed to fetch PRs";
  ...
  const loadIssues = async (owner: string, repo: string) => {
    ...
    try {
      ...
      const errorMsg =
        err.response?.data?.detail || err.message || "Failed to fetch issues";
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          {(prs.length > 0 || issues.length > 0) && (
            <div className="px-6 py-3 bg-gray-50 border-t border-gray-200 text-center text-xs text-gray-500">
              Summaries generated by Claude AI. Click on a card to expand
              details.
            </div>
          )}
        </div>
